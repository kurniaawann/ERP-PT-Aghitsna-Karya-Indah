...
class ExpenseReportController extends Controller
{
    ...
    public function destroySelected(Request $request)
    {
        ...
        try {
            ...
            // Get all selected expense reports
            ...
            if ($deletedCount > 0) {
                $message = "Berhasil menghapus {$deletedCount} data pengeluaran.";
                if ($skippedCount > 0) {
                    $message .= " ({$skippedCount} data auto-generated tidak dapat dihapus)";
                }
                return redirect()->route('expense-report.index')
                    ->with('success', $message);
            } else {
                return back()->with('warning', 'Tidak ada data yang dapat dihapus. Semua data yang dipilih adalah data auto-generated.');
            }
        } catch (\Exception $e) {
            DB::rollBack();
