<script>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        // ==========================================
        // ADD ITEM - Dari Stok Toggle
        // ==========================================

        function toggleStockHandler(e) {
        ...
        function initPriceValidation(row) {
            ...
            function validatePrices() {
                ...
                if (capital >= selling && selling > 0) {
                    ...
                    const allValid = Array.from(document.querySelectorAll('.barang-item-row')).every(r => {
        ...
        function initStockValidation(row) {
            ...
            function validateStock() {
                ...
                if (isFromStock && availableStock > 0 && qty > availableStock) {
                    ...
                    const allStockValid = Array.from(document.querySelectorAll('.barang-item-row')).every(r => {
                    ...
                    const allPricesValid = Array.from(document.querySelectorAll('.barang-item-row')).every(r => {
                        const cap = parseCurrencyInput(r.querySelector('.barang-item-capital')?.value);
                        const sel = parseCurrencyInput(r.querySelector('.barang-item-selling')?.value);
                        return sel === 0 || cap < sel;
                    });
        ...
        if (document.querySelector('.add-barang-item')) {
            document.querySelector('.add-barang-item').addEventListener('click', function(e) {
                e.preventDefault();
                const itemsContainer = document.getElementById('barang-items-list-add');
                ...
                newItem.innerHTML = `
                    ...
                    <p class="barang-stock-warning text-error text-sm mt-2 hidden">
                        <span class="font-semibold">⚠️ Peringatan Stok:</span> <span class="barang-stock-warning-text">Stok Barang Tidak Cukup! Silahkan Sesuaikan Dengan Stok Yang Tersedia.</span>
                    </p>
        ...
        // Initialize searchable dropdown, price validation, stock validation for existing items
        document.querySelectorAll('.barang-item-row').forEach(row => {
            initSearchableDropdown(row);
        ...
        if (addModal) {
            ...
            if (addForm) {
                addForm.addEventListener('submit', function(e) {
                    const items = [];
                    const itemRows = document.querySelectorAll('.barang-item-row');

                    let hasInvalidPrice = false;
                    itemRows.forEach(row => {
                        const capital = parseCurrencyInput(row.querySelector('.barang-item-capital')?.value);
                        const selling = parseCurrencyInput(row.querySelector('.barang-item-selling')?.value);
                        if (capital >= selling && selling > 0) {
                    ...
                    itemRows.forEach(row => {
                        const fromStockCheck = row.querySelector('.barang-from-stock');
                        const hiddenSelect = row.querySelector('.barang-select-hidden');
                        const itemName = row.querySelector('.barang-item-name').value;
                        const qty = parseInt(row.querySelector('.barang-item-qty').value) || 0;
                        const capital = parseCurrencyInput(row.querySelector('.barang-item-capital').value);
                        const selling = parseCurrencyInput(row.querySelector('.barang-item-selling').value);
        ...
        function toggleEditStockHandler() {
            ...
            if (this.checked) {
                ...
                row.querySelector('.barang-id-item-hidden').value = '';
            }
        }

        // ==========================================
        // EDIT ITEM - Price Validation
        // ==========================================
        ...
        function initPriceValidationEdit(row, invoiceNumber) {
            ...
            function validatePrices() {
                ...
                if (capital >= selling && selling > 0) {
                    ...
                    if (modalContainer) {
                        const allValid = Array.from(modalContainer.querySelectorAll('.barang-item-row-edit')).every(
                            r => {
        ...
        function removeEditItemHandler(e) {
            ...
            itemsContainer.querySelectorAll('.barang-item-row-edit').forEach((row, index) => {
        ...
        // ==========================================
        // EDIT ITEM - Attach Listeners
        // ==========================================

        function attachEditListeners() {
            document.querySelectorAll('.remove-barang-item-edit').forEach(btn => {
                btn.removeEventListener('click', removeEditItemHandler);
                btn.addEventListener('click', removeEditItemHandler);
            });

            document.querySelectorAll('.barang-from-stock-edit').forEach(checkbox => {
                checkbox.removeEventListener('change', toggleEditStockHandler);
                checkbox.addEventListener('change', toggleEditStockHandler);
            });

            document.querySelectorAll('.barang-item-row-edit').forEach(row => {
                initSearchableDropdownEdit(row);
            });
        }

        // ==========================================
        // EDIT ITEM - Add Item Button
        // ==========================================

        document.querySelectorAll('.add-barang-item-edit').forEach(btn => {
            btn.addEventListener('click', function(e) {
                e.preventDefault();
                const invoiceNumber = this.getAttribute('data-invoice-number');
                const itemsContainer = document.getElementById('barang-items-list-edit-' + invoiceNumber);
                const currentItems = itemsContainer.querySelectorAll('.barang-item-row-edit');
                ...
                newItem.innerHTML = `
                            ...
                            <div class="barang-options-edit">
                                ...
                                @foreach ($items as $stockItem)
                                    <div class="p-3 hover:bg-primary-light cursor-pointer border-b border-border-light barang-option-edit"
                                        ...
                                        <div class="font-medium text-text-heading">{{ $stockItem->name_item }}</div>
                                        <div class="text-xs text-text-secondary mt-1">
                                            Stok: <span class="font-semibold text-primary">{{ $stockItem->quantity }}</span> unit
                                        </div>
                                    </div>
                                @endforeach
                            </div>
                    ...
                    <input type="text" name="items[${newIndex}][name_item]"
                        class="barang-item-name-edit w-full border rounded p-2 mb-2" placeholder="Nama Barang *" required
                        oninvalid="this.setCustomValidity('Nama barang tidak boleh kosong')"
                        oninput="this.setCustomValidity('')">
                    
                    <div class="grid grid-cols-3 gap-2">
                        <input type="number" name="items[${newIndex}][quantity]"
                            class="barang-item-qty-edit border rounded p-2" placeholder="Qty *" required min="1" value="1"
                            oninvalid="this.setCustomValidity('Qty tidak boleh kosong')"
                            oninput="this.setCustomValidity('')">
                        <input type="text" inputmode="numeric" name="items[${newIndex}][capital_price]"
                            class="barang-item-capital-edit border rounded p-2" placeholder="Rp 0" required value="Rp 0"
                            oninvalid="this.setCustomValidity('Harga modal tidak boleh kosong')"
                            oninput="formatCurrencyInput(this); this.setCustomValidity('')">
                        <input type="text" inputmode="numeric" name="items[${newIndex}][selling_price]"
                            class="barang-item-selling-edit border rounded p-2" placeholder="Rp 0" required value="Rp 0"
                            oninvalid="this.setCustomValidity('Harga jual tidak boleh kosong')"
                            oninput="formatCurrencyInput(this); this.setCustomValidity('')">
                    </div>
                    ...
                    <p class="barang-stock-warning-edit text-error text-sm mt-2 hidden">
                        <span class="font-semibold">⚠️ Peringatan Stok:</span> <span class="barang-stock-warning-text-edit">Stok Barang Tidak Cukup! Silahkan Sesuaikan Dengan Stok Yang Tersedia.</span>
                    </p>
        ...
        editForms.forEach(function(form) {
            form.addEventListener('submit', function(e) {
                ...
                const invoiceNumber = modal ? modal.id.replace('editModal-', '') : '';
                const itemsContainer = document.getElementById('barang-items-list-edit-' + invoiceNumber);

                if (itemsContainer) {
                    let hasInvalidPrice = false;
                    itemsContainer.querySelectorAll('.barang-item-row-edit').forEach(row => {
                        const capital = parseCurrencyInput(row.querySelector('.barang-item-capital-edit')?.value);
                        const selling = parseCurrencyInput(row.querySelector('.barang-item-selling-edit')?.value);
                        if (capital >= selling && selling > 0) {
                            hasInvalidPrice = true;
                        }
                    });
        ...
        // ==========================================
        // INITIALIZE EDIT MODAL ITEMS
        // ==========================================

        attachEditListeners();

        // Initialize price and stock validation for existing edit items
        document.querySelectorAll('[id^="barang-items-list-edit-"]').forEach(container => {
            const invoiceNumber = container.id.replace('barang-items-list-edit-', '');
            container.querySelectorAll('.barang-item-row-edit').forEach(row => {
