...
@section('content')
    <div class="bg-surface-base p-4 sm:p-6 rounded-xl shadow">
        ...
        <div class="mb-4 flex items-center justify-between flex-wrap gap-3">
            {{-- Form Search --}}
            <form method="GET" action="{{ route('rab.index') }}" class="w-full min-[1530px]:w-auto min-[1530px]:flex-1 flex flex-col min-[1530px]:flex-row gap-3">
                <x-filters.month-filter :value="request('month')" responsive="custom" />
                <x-filters.year-filter :value="request('year')" responsive="custom" />
                <x-filters.search-input :value="request('search')" placeholder="Cari nomor RAB / penerima..." responsive="custom" />
            </form>
