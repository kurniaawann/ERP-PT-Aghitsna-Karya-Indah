...
@section('content')
    ...
    @foreach ($salesReports as $sale)
        ...
        {{-- Modal Update Status --}}
        @if (!$sale->isLunas())
            <x-modal id="statusModal-{{ $sale->id_sales_report }}" title="Update Status"
                action="{{ route('sales-report.updateStatus', $sale->id_sales_report) }}" method="POST"
                buttonText="Update Status">
                @method('PATCH')

                <div class="mb-4">
                    <p class="text-gray-700 mb-3">Update status laporan penjualan:</p>
                    <p class="font-semibold mb-2">{{ $sale->name_proyek }}</p>
                    <p class="text-sm text-gray-600 mb-4">Tanggal: {{ $sale->date->format('d-m-Y') }}</p>

                    <label class="block text-gray-700 mb-2">Status <span class="text-error">*</span></label>
                    ...
                    <p class="text-xs text-warning mt-2">
                        ...
                        Perhatian: Setelah status diubah menjadi "Lunas", data tidak dapat diubah lagi.
                    </p>
                </div>
            </x-modal>
        @endif
