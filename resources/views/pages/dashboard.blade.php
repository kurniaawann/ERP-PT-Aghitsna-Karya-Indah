            ...
            <div class="bg-warning-light px-6 py-4 border-b border-warning-light">
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        ...
                        <p class="text-sm text-text-secondary">Karyawan dengan gaji belum dibayar (Minggu 1 - {{ $currentWeek }})</p>
                    </div>
                </div>
            </div>
                            ...
                            <div class="flex items-start justify-between p-3 bg-warning-light rounded-lg border border-warning-light hover:bg-white transition-colors">
                                <div class="flex items-start gap-3 flex-1">
                                    ...
                                    <div class="flex-1">
                                        ...
                                        <p class="text-sm text-text-secondary">{{ $item['employee']->employee_code }} - {{ $item['employee']->position }}</p>

                                        <div class="flex flex-wrap gap-1 mt-2">
                                            @foreach ($item['unpaid_weeks'] as $unpaidWeek)
                                                <span class="inline-flex items-center px-2 py-1 rounded-md text-xs font-medium bg-red-100 text-red-700 border border-red-200">
                                                    <svg xmlns="http://www.w3.org/2000/svg" class="h-3 w-3 mr-1" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4m0 4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
                                                    </svg>
                                                    Minggu {{ $unpaidWeek['week_number'] }} ({{ $unpaidWeek['start_date'] }}-{{ $unpaidWeek['end_date'] }})
                                                </span>
                                            @endforeach
                                        </div>
                                    </div>
                                ...
                                <div class="text-right ml-3">
                                    <span class="text-sm font-medium text-warning whitespace-nowrap">
                                        Rp {{ number_format($item['employee']->base_salary, 0, ',', '.') }}
                                    </span>
                                </div>
                            </div>
                        @endforeach
                        ...
                        @foreach ($lowStockItems as $item)
                            <div class="flex items-center justify-between p-3 bg-error-light rounded-lg border border-error-light hover:bg-white transition-colors">
                                ...
                                <div class="text-right">
                                    <span class="inline-flex items-center px-3 py-1 rounded-full text-sm font-medium {{ $item->quantity == 0 ? 'bg-error-light text-error' : ($item->quantity <= 2 ? 'bg-warning-light text-warning' : 'bg-info-light text-info') }}">
                                        {{ $item->quantity }} unit
                                    </span>
                                </div>
                            </div>
