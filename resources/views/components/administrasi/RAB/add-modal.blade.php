...
<x-modal id="addRABModal" title="Tambah RAB (Rancangan Anggaran Biaya)" action="{{ route('rab.store') }}" method="POST"
    ...
    <div class="mb-3">
        ...
        <input type="text" class="w-full border border-border-strong rounded p-2 bg-surface-base text-text-input"
            name="project_name" placeholder="Nama proyek (otomatis menjadi Rekap Proyek)" required maxlength="255"
            oninvalid="this.setCustomValidity('Nama proyek tidak boleh kosong')" oninput="this.setCustomValidity('')">
        <small class="text-text-secondary text-xs">Rekap Proyek akan dibuat otomatis dengan total dari RAB ini.</small>
    </div>
