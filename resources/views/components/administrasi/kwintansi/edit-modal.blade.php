...
<x-modal id="editModal-{{ $kwintansi->id_kwintansi }}" title="Edit Kwintansi"
    ...
    <div class="mb-3">
        ...
        <textarea name="payment_for" class="w-full border rounded p-2" rows="3" required
            oninvalid="this.setCustomValidity('Keterangan tidak boleh kosong')" oninput="this.setCustomValidity('')">{{ $kwintansi->payment_for }}</textarea>
    </div>
        ...
        <div>
            ...
            <select name="signed_by_id" class="w-full border rounded p-2">
                ...
                @foreach ($executives ?? [] as $executive)
                    <option value="{{ $executive->id }}" @selected($kwintansi->signed_by_id == $executive->id)>
                        {{ $executive->name }}
                    </option>
                @endforeach
            </select>
    ...
    @php
        $selectedAccounts = $kwintansi->selected_payment_accounts ?? [];
        if (empty($selectedAccounts) && $kwintansi->payment_account_id) {
            $selectedAccounts = [$kwintansi->payment_account_id];
        }
    @endphp
