{{-- Invoice Semen Table Component --}}
<form id="deleteForm" method="POST" action="{{ route('semen-invoice.destroySelected') }}">
    @csrf
    @method('DELETE')
                ...
                <table class="min-w-full divide-y divide-gray-200">
                    <thead class="bg-gradient-to-r from-gray-50 to-gray-100">
                        <tr>
                            ...
                            <th class="p-2 text-center">Jml Proyek</th>
                    ...
                    <tbody>
                        @forelse ($invoices as $invoice)
                            <tr class="border-t hover:bg-surface-secondary">
                                ...
                                <td class="p-2 text-text-label">
                                    {{ $invoice->nama_proyek_list ? \Illuminate\Support\Str::limit($invoice->nama_proyek_list, 40) : '-' }}
                                </td>
                                <td class="p-2 text-center">{{ $invoice->proyek_count }}</td>
                                ...
                                <td class="p-2 text-center">
                                    <div class="flex justify-center gap-[5px] flex-wrap min-[1363px]:grid min-[1363px]:gap-[5px] min-[1363px]:grid-cols-[auto_auto] min-[1363px]:w-fit min-[1363px]:mx-auto min-[1436px]:grid-cols-[auto_auto_auto_auto] min-[1436px]:gap-[5px]">
                                        ...
                                        <button type="button"
                                            onclick="openEditSemenModal('{{ $invoice->invoice_number }}')"
                                        ...
                                        <a href="{{ route('semen-invoice.print.excel', $invoice->invoice_number) }}"
                        ...
                        @empty
                            <tr>
                                <td colspan="7" class="text-center p-4 text-text-secondary">Data invoice semen tidak
                                    ditemukan.</td>
                            </tr>
                        @endforelse
                    </tbody>
